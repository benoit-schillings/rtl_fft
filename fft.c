...
void init_sdr(int dev_index)
{
    ...
    r = rtlsdr_set_center_freq(dev, 0135.270*1e6);
    r = rtlsdr_set_tuner_gain_mode(dev, 1);
    r = rtlsdr_set_tuner_gain(dev, 336);
	...
	do {
		...
		if (cnt == SIZE * 2) {
            ...
            if (buffer_count == (32)) {
                ...
                float max = 0;
                for (int i = 0; i < SIZE/2; i++) {
                    if (fft_sum[i*2]>max) max = fft_sum[i*2];
                }
                printf("%f\n", max-2900.0);
                for (int i = 0; i < SIZE; i++) {
                    fft_sum[i] = 0;
...
int demod_from_source_main(int argc, char **argv)
{
    float   *signal;
    float   freq = 5000;
    
    
    signal = (float *)malloc(sizeof(float) * SIZE);
	...
	do {
        ...
        demod(data, signal, SIZE, freq, DEFAULT_SAMPLE_RATE);
        
        float   *s = signal;
        
        int ratio = DEFAULT_SAMPLE_RATE/44000;
